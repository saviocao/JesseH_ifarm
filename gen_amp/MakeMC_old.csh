...
#copy config locally
	...
	if ( "$GENERATOR_POST" == "decay_evtgen" ) then
		...
		echo decay_evtgen -o$STANDARD_NAME'_decay_evtgen'.hddm -upostdec'_'$GENERATOR_POST'_'$formatted_runNumber'_'$formatted_fileNumber.conf $STANDARD_NAME.hddm
		decay_evtgen -o$STANDARD_NAME'_decay_evtgen'.hddm -upostdec'_'$GENERATOR_POST'_'$formatted_runNumber'_'$formatted_fileNumber.conf $STANDARD_NAME.hddm
    ...
    if ( "$GEANT" != "0"  ) then
		...
		#change lambda br definitions to only get lambda->pi- p
		echo "/particle/select lambda" >>! run.mac
		echo "/particle/property/decay/select 0" >>! run.mac
                echo "/particle/property/decay/br 1" >>! run.mac
		echo "/particle/property/decay/select 1" >>! run.mac
                echo "/particle/property/decay/br 0" >>! run.mac
		echo "/particle/property/decay/dump" >>! run.mac
    ...
    setenv RCDB_CONNECTION sqlite:////group/halld/www/halldweb/html/dist/rcdb.sqlite
					...
					if ( "$CUSTOM_ANA_PLUGINS" != "None" ) then
						if ( $ana_pre == "file" ) then
							...
							set anapluginlist=( "monitoring_hists" "$CUSTOM_ANA_PLUGINS" )
							set anaPluginStr=""
							foreach plugin ($anapluginlist)
									set anaPluginStr="$anaPluginStr""$plugin"","
							end
							set anaPluginStr=`echo $anaPluginStr | sed -r 's/.{1}$//'` #remove last ","
					...
					set thrown_tree_check=`grep mcthrown_tree ana_jana.cfg`
