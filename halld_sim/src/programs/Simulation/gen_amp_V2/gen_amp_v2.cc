...
int main( int argc, char* argv[] ){
  ...
  double lvLowMass   = 0.0;
  double lvHighMass  = 3.0;
  ...
  for (int i = 1; i < argc; i++){
	...
	lvLowMass = atof( argv[i+1] );
	lvHighMass = atof( argv[i+2] );
      }
    }
    if (arg == "-uvRange"){
      if ((i+1 == argc) || (i+2 == argc) || (argv[i+1][0] == '-') || (argv[i+2][0] == '-')) arg = "-h";
  ...
  AmpToolsInterface::registerAmplitude( Hist3D() );
  ...
  TH1F* m_recoil = new TH1F( "m_recoil", locRecoilTitle.c_str(), 200, lvLowMass, lvHighMass );
  TH1F* mW_recoil = new TH1F( "mW_recoil", locRecoilTitle.c_str(), 200, lvLowMass, lvHighMass );

  TH2F* M_CosTheta = new TH2F( "M_CosTheta", "M vs. cos#vartheta", 200, lvLowMass, lvHighMass, 200, -1, 1);
  TH2F* MW_CosTheta = new TH2F( "MW_CosTheta", "M vs. cos#vartheta", 200, lvLowMass, lvHighMass, 200, -1, 1);
  ...
  while( eventCounter < nEvents ){
    ...
    for( int i = 0; i < batchSize; ++i ){
      ...
      TLorentzVector res = evt->particle( 2 );
	...
	if( weightedInten > rand){
	  ...
	  t->Fill( -1*(beam-resonance).M2() );

   	  tW->Fill( -1*(beam-resonance).M2(),genWeight );	  
	  ...
	  TLorentzVector rec_com = comBoost * recoil;
	  TLorentzVector res_com = comBoost * res;
	  
	  TLorentzRotation recoilBoost( -rec_com.BoostVector() );
	  TLorentzVector beam_rec = recoilBoost * beam_com;
	  TLorentzVector rec_rec = recoilBoost * rec_com;
	  TLorentzVector res_rec = recoilBoost * res_com;
	  
	  // normal to the production plane
	  TVector3 y = (beam_com.Vect().Cross(rec_com.Vect())).Unit();
	  
	  // choose helicity frame: z-axis opposite recoil proton in rho rest frame
	  TVector3 z = 1. * rec_com.Vect().Unit();
	  TVector3 x = y.Cross(z).Unit();
	  TVector3 angles( (res_rec.Vect()).Dot(x),
			   (res_rec.Vect()).Dot(y),
			   (res_rec.Vect()).Dot(z) );
	  
	  double cosTheta = angles.CosTheta();
	  
	  //cout << "Ystar (px,py,pz,E) costheta: (" << res_rec.Px() << "," << res_rec.Py() << "," << res_rec.Pz() << "," << res_rec.E() << ") " << cosTheta << ")" << endl;  
	  //cout << "Target (px,py,pz,E): (" << target.Px() << "," << target.Py() << "," << target.Pz() << "," << target.E()  << ")" << endl;  

	  M_CosTheta->Fill( recoil.M(), cosTheta);
	  MW_CosTheta->Fill( recoil.M(), cosTheta, genWeight);
	...
	intenW->Fill( weightedInten );
	intenWVsE->Fill( beam.E(), weightedInten );	
	intenWVsM->Fill( recoil.M(), weightedInten );
