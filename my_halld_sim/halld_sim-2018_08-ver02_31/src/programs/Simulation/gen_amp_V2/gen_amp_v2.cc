...
int main( int argc, char* argv[] ){
  ...
  while( eventCounter < nEvents ){
    ...
    for( int i = 0; i < batchSize; ++i ){
      ...
      TLorentzVector res = evt->particle( 3 );
	...
	if( weightedInten > rand){
	  ...
	  TLorentzVector beam_rec = recoilBoost * beam;
	  TLorentzVector rec_rec = recoilBoost * recoil;
	  TLorentzVector res_rec = recoilBoost * res;
	  
	  // normal to the production plane
	  TVector3 y = (beam.Vect().Cross(recoil.Vect())).Unit();
	  
	  // choose helicity frame: z-axis opposite recoil proton in rho rest frame
	  TVector3 z = 1. * recoil.Vect().Unit();
	  TVector3 x = y.Cross(z).Unit();
	  TVector3 res_rec_p3( (res_rec.Vect()).Dot(x),
			       (res_rec.Vect()).Dot(y),
			   (res_rec.Vect()).Dot(z) );
	  
	  double cosTheta = res_rec_p3.Pz() / res_rec_p3.Mag();
	  
	  //cout << "Are we in COM frame(beam, p1,resonance): (" << beam.Pz() << ", "<< resonance_com.Pz() << ", " << rec_com.Pz() << ")" << endl;
