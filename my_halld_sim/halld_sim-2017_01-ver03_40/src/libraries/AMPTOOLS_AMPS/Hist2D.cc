...
Hist2D::calcAmplitude( GDouble** pKin ) const {
	...
	TLorentzRotation resRestBoost( -res.BoostVector() );
	...
	TVector3 z = 1 * res.Vect().Unit();

	// y axis perpendicular to production plane
	TVector3 y = (beam.Vect().Cross(res.Vect())).Unit();

	TVector3 x = y.Cross(z);
  
	TVector3 p2_p ( p2_res.Vect().Dot(x),
			 p2_res.Vect().Dot(y),
			 p2_res.Vect().Dot(z) );
  
	double cosTheta = p2_p.Pz() / p2_p.Mag();

	//cout << "Beam P(initial, com): " << beam.P() << ", " << beam_com.P() << endl;
	//cout << "Checking for CoM frame(Recoil, Res): (" << recoil_com.Pz() << ", " << res_com.Pz() << ")" << endl;
